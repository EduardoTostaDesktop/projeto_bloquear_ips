...
def criar_solicitacao(request):
    if request.method == "POST":
        ...
        if lista_existente_id:
            ...
            if not nome_lista:
                messages.error(request, "Digite um nome para a nova lista ou selecione uma existente.")
                return redirect("solicitacoes:criar_solicitacao")
            lista = Lista.objects.create(
                ...
                obs=obs,
                criador=usuario,
                data_prevista_desbloqueio=data_prevista
            )
            # Processa arquivo Excel se houver
            if arquivo:
                try:
                    import openpyxl
        ...
        # Vincula endereços selecionados manualmente
        if enderecos_ids:
            for eid in enderecos_ids:
                endereco_obj = get_object_or_404(Endereco, id=eid)
                lista.enderecos.add(endereco_obj)
        ...
        # Bloqueia ou desbloqueia endereços automaticamente
        if tipo == "BLOQUEIO":
